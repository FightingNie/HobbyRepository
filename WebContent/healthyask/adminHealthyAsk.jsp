			...
			<div class="col-xs-12 col-sm-9">
				...
				<table class="table table-hover">
					...
					<c:forEach var="item" items="${healthyAskList}">
						<tr>
							<td>${item.title}</td>
							<td><a data-toggle="modal" data-target="#healthyQuestionModal" data-question="${item.question}" data-title="${item.title}" data-itemid="${item.id}" href="#">${item.question}</a></td>
	      ...
	      <div class="modal-body">
	        ...
	        <form id="answerForm">
		        <div class="form-group">
					...
					<label>回答：</label> <textarea required
						class="form-control name" name="answer" placeholder="输入答案"></textarea>
				</div>
			</form>
	      </div>
	      <div class="modal-footer">
	        <button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
	        <button type="button" class="btn btn-primary" onclick="healthyAsk.answer()">答复问题</button>
	      </div>
	    </div>
	  </div>
	</div>
