		...
		<div class="row row-offcanvas row-offcanvas-right">
			<div class="col-xs-12 col-sm-9">
				...
				<table class="table table-hover">
					<tr><th>小区名称</th><th>上车点</th><th>下车点</th><th>人数</th><th>时间</th><th>路费</th><th>发起人</th><th>联系方式</th><th>详情</th><th>给发起人留言</th></tr>
					<c:forEach var="item" items="${carHugList}">
						<tr>
							...
							<td>${item.userMessage.societyName}</td><td>${item.carHug.inCarAddr}</td><td>${item.carHug.outCarAddr}</td><td>${item.carHug.seatNumber}</td><td>${item.carHug.hugTime}</td>
							<td>${item.carHug.tripMoney}</td><td>${item.userMessage.nickName}</td><td>${item.carHug.phoneNumber}</td>
							<!-- <td><a data-toggle="modal" data-target="#showHugModal" data-itemid="${item.carHug.id}" data-makecarhugusername="${item.userMessage.nickName}" data-membersstr="${item.carHug.membersStr}" href="#">详情</a></td> -->
							<td><a href="carHugListDetail?hugId=${item.carHug.id}&makeCarHugUsername=${item.userMessage.nickName}">详情</a></td>
							<!-- <td><a data-toggle="modal" data-target="#leaveHugMessModal" data-itemid="${item.carHug.id}" data-userid="${item.userMessage.id}" data-membersstr="${item.carHug.membersStr}" href="#">留言</a></td> -->
							<td><a href="submitMessageDemo?recieveUserId=${item.userMessage.id}&type=3">留言</a></td>
						</tr>
					</c:forEach>
				</table>
				</div>
