		...
		<div class="row row-offcanvas row-offcanvas-right">
			<div class="col-xs-12 col-sm-9">
				...
				<table class="table table-hover">
					<tr><th>活动主题</th><th>活动时间</th><th>活动地点</th><th>详情</th><th>是否已参加</th></tr>
					<c:forEach var="item" items="${healthyActivityList}">
						<tr>
							<td>${item.activityTheme}</td><td>${item.activityTime}</td><td>${item.activityAddr}</td>
							<td><a data-toggle="modal" data-target="#healthyActivityModal" data-theme="${item.activityTheme}" data-content="${item.activityContent}" data-membersstr="${item.membersStr}" data-time="${item.activityTime }" data-addr="${item.activityAddr}" data-itemid="${item.id}" href="#">详情</a></td>
							<td>${item.isPart}</td>
						</tr>
					</c:forEach>
				</table>
	  ...
	  <div class="modal-dialog" role="document">
	    <div class="modal-content">
	      ...
	      <div class="modal-footer">
	       ...
	       <!-- <button type="button" class="btn btn-primary"  onclick="javascript:window.location.href('takePartInActivity')">参加活动</button> -->
	        <form id="takePartInForm" action="takePartInActivity" class="btn btn-default">
	        	<input name="healthyActivityId" type="hidden" class="healthyActivityId"/>
	        	<input type="submit" name="submit" value="参加活动"/>
	        </form>  
	      </div>
	    </div>
	  </div>
