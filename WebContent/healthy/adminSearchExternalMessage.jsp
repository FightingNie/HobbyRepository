	...
	<div class="container">
		<div class="row row-offcanvas row-offcanvas-right">
			...
			<div class="table-responsive">
				...
				${message}
					<tr><th>身高</th><th>体重</th><th>皮肤</th><th>脊柱四肢</th><th>其他</th><th>结论</th><th>外科检查员</th></tr>
						<tr>
							<td>${exm.height}cm</td><td>${exm.weight}kg</td>
							<td>${exm.skin}</td><td>${exm.extremitiesAndSpine}</td>
							<td>${exm.others}</td><td>${exm.comment}</td>
							<td>${exm.externalInspector}</td>
							<!--
							<td><a href="sdas.jsp">删除</a></td> 
							data-toggle="modal" data-target="#confirmModal" data-itemid="${em.id}"  -->
						</tr>
				</table>
				<c:if test="${!empty exm}">
				<button class="btn btn-primary" data-toggle="modal" data-target="#confirmModal" data-itemid="${exm.id}">删除</button>
				<a href="updateExternalMessageDemo?id=${exm.id}" class="btn btn-primary">修改记录</a>
				</c:if>
				<button class="btn btn-primary" data-toggle="modal" data-target="#publishModal">添加记录</button>
				</div>
	  ...
	  <div class="modal-dialog" role="document">
	    <div class="modal-content">
	      ...
	      <div class="modal-body">
	        ...
	        <button type="button" class="btn btn-primary" onclick="healthyActivity.delExternalMessage()">确认</button>
	      </div>
	        </form>
	      </div>
	    </div>
	...
	<div class="modal fade" id="publishModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
	  <div class="modal-dialog" role="document">
	    <div class="modal-content">
	      <div class="modal-header">
	        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
	        <h4 class="modal-title" id="myModalLabel">外科记录</h4>
	      </div>
	      <div class="modal-body">
	      	<form id="publishForm">
		        ...
		        <div class="form-group">
					<label>身高：</label> <input type="text"
						class="form-control" name="height" placeholder="身高"/>cm
				</div>
				<div class="form-group">
					<label>体重：</label> <input type="text"
						class="form-control" name="weight" placeholder="体重"/>kg
				</div>
				<div class="form-group">
					<label>皮肤：</label> <textarea 
						class="form-control" name="skin" placeholder="皮肤"></textarea>
				</div>
				<div class="form-group">
	      ...
	      <div class="modal-footer">
	        <button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
	        <button type="button" class="btn btn-primary" onClick="healthyActivity.submitExternalMessage()">提交</button>
	      </div>
	    </div>
	  </div>
	</div>
